...
export async function runTool(name, args, user) {
  try {
    ...
    switch (name) {
      case "list_classes": {
        const classes = (await listWorkshops()).map(classSummary);
        return { classes };
      }

      case "list_my_bookings": {
        const bookings = (await listBookingsForUser(user.id)).map((b) => ({
          id: b.id,
          title: b.title,
      ...
      case "create_booking": {
        ...
        if (!query.trim()) {
          return {
            ...
            validClasses: (await listWorkshops()).map((w) => w.title),
          };
        }
        const w = await findWorkshopByQuery(query);
        if (!w) {
          return {
        ...
        const { booking, alreadyBooked } = await createBooking({
          userId: user.id,
          workshopId: w.id,
        });
        return {
