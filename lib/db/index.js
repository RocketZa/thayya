...
export async function addTrack(instructorId, { title, artist, duration, mood, bpm, source, audioUrl }) {
  const track = {
    id: newId("trk"),
    instructorId,
    ...
    audioUrl: typeof audioUrl === "string" && audioUrl.trim() ? audioUrl.trim().slice(0, 300) : null,
    createdAt: Date.now(),
  };
