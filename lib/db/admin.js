...
// Lightweight instructor list for pickers: { instructorId, name }.
// Only instructors that actually have an instructorId set are returned.
export async function listInstructorOptions() {
  const dbi = getDb();
  const rows = await dbi
    .select({ instructorId: users.instructorId, name: users.name })
    .from(users)
    .where(eq(users.role, "instructor"))
    .orderBy(asc(users.name));
  return rows.filter((r) => r.instructorId).map((r) => ({ instructorId: r.instructorId, name: r.name }));
}

// True when the given instructorId belongs to a real instructor user.
export async function instructorExists(instructorId) {
  if (!instructorId) return false;
  const dbi = getDb();
  const rows = await dbi
    .select({ instructorId: users.instructorId })
    .from(users)
    .where(and(eq(users.role, "instructor"), eq(users.instructorId, instructorId)))
    .limit(1);
